...
remotes::install_github("r4ss/r4ss", ref="1.26.0") # install r4ss version 1.26.0
...
mydir <- file.path(path.package("r4ss"),
                   file.path("extdata", "simple_3.30.13"))
...
unparsable by Stock Synthesis, the `SS_write()` functions may throw an
...
An advantage of `run()` is that there is no need to change the working
directory.

If the executable in a different folder than the model, specify either
the absolute or relative path to the executable. In the situation that
the executable is in the working directory, it is necessary to provide
the absolute path:
```{r exe-in-diff-dir, eval=FALSE}
# get the absolute exe path
exe_path <- normalizePath("ss.exe", winslash = "/", mustWork = TRUE)
# use the absolute exe path in the call.
run(dir = new_mod_path, model = exe_path, skipfinished = FALSE)
```

Finally, if the stock synthesis executable is in your PATH, then the the
`run()` can be called with `exe_in_path = TRUE`.

```{r run-mult-models, eval=FALSE}
r4ss::run(dir = new_mod_path, exe_in_path = TRUE, skipfinished = FALSE)
```

### Running models in multiple directories

A vector of directories can also be passed to the `dir` variable in
`run()` to run models in multiple directories with one function call.
...
functions to perform a retrospective (`SS_doRetro()`), jitter the
starting values and reoptimize the stock assessment model a number of
times to check for local minima (`SS_run_jitter()`) and tuning
composition data (`SS_tune_comps()`).
...
The function `SS_doRetro()` can be used to run retrospective analyses
...
copy_SS_inputs(dir.old = mod_path, dir.new = retro_dir) 

# run the retrospective analyses
SS_doRetro(masterdir = retro_dir, 
           oldsubdir = "", 
           newsubdir = "retrospectives", 
           years = 0:-5)
```
After running this retrospective, six new folders would be created
within `retro_dir`, where each folder would contain a different run of
the retrospective (removing 0 to 5 years of data).
...
fraction) and the model is reoptimized. The `SS_RunJitter()` function
performs this routine for the number of times specified by the user. For
a stock Synthesis model in a folder called `my_model` that also contains
a copy of the stock synthesis executable named ss (or ss.exe on
Windows), jittering starting values can be run 100 times (note this
...
```
The output from `SS_RunJitter()` is saved in `jitter_loglike`, which is
...
tuning methods, `SS_tune_comps()` can start from the same model and
...
# Path to simple_3.30.13 model in r4ss
...
The following call to `SS_tune_comps()` runs Francis weighting for 1
...
to a new folder, then the `SS_tune_comps()` function could be used to
...
There are many options in the `SS_tune_comps()` function; please see the
documentation (`?SS_tune_comps` in the R console) for more details and
examples.
