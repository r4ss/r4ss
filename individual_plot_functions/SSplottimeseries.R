SSplottimeseries <-
  function(replist,subplots=1:5,add=F,areas="all",areacols=1:10,areanames="default",
           forecastplot=T,uncertainty=T,bioscale="default",
           plot=T,print=F,plotdir="default",verbose=F,
           xlab="Year",
           ...
           pwidth=7,pheight=7,punits="in",res=300,ptsize=12)
{
  ...
  biofunc <- function(subplot){
    ...
    if(print){ # if printing to a file
      ...
      ## filename <- gsub("__","_",filename,fixed=T)
    ...
    abline(h=0,col="grey")
    if(verbose) print(paste("finished time series subplot ",subplot,": ",main,sep=""))
    if(print) dev.off()
  } # end biofunc
