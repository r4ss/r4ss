...
#' @author Megan Stachura, Andrew Cooper, Andi Stephens, Neil Klaer, Ian Taylor
#' @export
#' @keywords cryptic
           ...
           pwidth=6.5,pheight=5.0,punits="in",res=300,ptsize=10,cex.main=1)
{
  ...
  #   age.selectivity <- replist$ageselex
  #   age.selectivity.female <- subset(replist$ageselex, gender==1 & 
  #                                      factor=='Asel2' & 
  #                                      year==max(replist$ageselex$year))
    ...
    for(y in 1:nrow(catch.by.fleet)){
      ...
      age.selectivity.female.year <- subset(replist$ageselex, gender==1 & 
                                         factor=='Asel2' & 
                                         year==year.get &
  ...
  #                las=1, x.hash=years[1])
      ...
      par(xpd=NA)
      plot(1,1, type='n', ylim=c(0,replist$accuage+4), 
      ...
      symbols(x = min(years.with.catch) + 0.3*(max(years.with.catch) - min(years.with.catch)), 
              y = replist$accuage+larger.circle*multiplier, 
      ...
      symbols(x = min(years.with.catch) + 0.2*(max(years.with.catch) - min(years.with.catch)), 
              y = replist$accuage+smaller.circle*multiplier, 
      ...
      plot(0,0, type='n', ylim=c(0,1.1), xlim=c(0, replist$accuage), 
      ...
      for(i in 1:nrow(mean.selectivity)){
        lines(0:replist$accuage, mean.selectivity[i,], col=cols[i])
      }
