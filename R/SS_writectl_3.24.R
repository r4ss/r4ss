...
SS_writectl_3.24 <- function(
  ...
  if (ctllist[["N_GP"]] * ctllist[["nseas"]] * ctllist[["N_areas"]] > 1) {
    wl(
      "recr_dist_read",
      comment = "#_number of recruitment assignments (overrides GP*area*seas parameter values)"
    )
  ...
  if (ctllist[["N_areas"]] > 1) {
    ...
    wl(
      "firstAgeMove",
      comment = "#_first age that moves (real age at begin of season, not integer) also cond on do_migration>0"
    )
  ...
  wl(
    "natM_type",
    comment = "#_natM_type:_0=1Parm; 1=N_breakpoints;_2=Lorenzen;_3=agespecific;_4=agespec_withseasinterpolate"
  )
  ...
  wl(
    "GrowthModel",
    comment = "# GrowthModel: 1=vonBert with L1&L2; 2=Richards with L1&L2; 3=age_speciific_K; 4=not implemented"
  )
  ...
  wl(
    "CV_Growth_Pattern",
    comment = "#_CV_Growth_Pattern:  0 CV=f(LAA); 1 CV=F(A); 2 SD=F(LAA); 3 SD=F(A); 4 logSD=F(A)"
  )
  wl(
    "maturity_option",
    comment = "#_maturity_option:  1=length logistic; 2=age logistic; 3=read age-maturity by GP; 4=read age-fecundity by GP; 5=read fec and wt from wtatage.ss; 6=read length-maturity by GP"
  )
  ...
  wl(
    ...
    comment = "parameter_offset_approach (1=none, 2= M, G, CV_G as offset from female-GP1, 3=like SS2 V1.x)"
  )
  wl(
    "env_block_dev_adjust_method",
    comment = "env/block/dev_adjust_method (1=standard; 2=logistic transform keeps in base parm bounds; 3=standard w/ no bound check)"
  )
  ...
  wl.vector(
    "MGparm_seas_effects",
    comment = "#_femwtlen1,femwtlen2,mat1,mat2,fec1,fec2,Malewtlen1,malewtlen2,L1,K"
  )
  ...
  wl(
    "SR_function",
    comment = "#_SR_function: 2=Ricker; 3=std_B-H; 4=SCAA; 5=Hockey; 6=B-H_flattop; 7=survival_3Parm; 8=Shepard_3Parm"
  )
  printdf("SR_parms")
  wl("SR_env_link")
  wl("SR_env_target")
  ...
  if (ctllist[["recdev_adv"]]) {
    wl(
      "recdev_early_start",
      comment = "#_recdev_early_start (0=none; neg value makes relative to recdev_start)"
    )
    wl("recdev_early_phase", comment = "#_recdev_early_phase")
    wl(
      "Fcast_recr_phase",
      comment = "#_forecast_recruitment phase (incl. late recr) (0 value resets to maxphase+1)"
    )
    wl(
    ...
    wl(
      ...
      comment = "#_last_early_yr_nobias_adj_in_MPD"
    )
    ...
    wl(
      ...
      comment = "#_first_recent_yr_nobias_adj_in_MPD"
    )
    wl(
      "max_bias_adj",
      comment = "#_max_bias_adj_in_MPD (-1 to override ramp and set biasadj=1.0 for all estimated recdevs)"
    )
    wl(
  ...
  wl(
    "F_ballpark",
    comment = "F ballpark for annual F (=Z-M) for specified year"
  )
  ...
  if (sum(ctllist[["Q_setup"]][, 4] %in% c(3, 4)) > 0) {
    wl(
      "Do_Q_detail",
      comment = "If q has random component, then 0=read one parm for each fleet with random q; 1=read a parm for each year of index"
    )
  } else {
    writeComment("#")
    writeComment(
  ...
  if (
    !is.null(ctllist[["selparm_Dev_Phase"]]) && ctllist[["selparm_Dev_Phase"]]
  ) {
  ...
  if (ctllist[["DoAdjust"]]) {
    wl(
      "selex_adjust_method",
      comment = "env/block/dev_adjust_method (1=standard; 2=logistic trans to keep in base parm bounds; 3=standard w/ no bound check)"
    )
  } else {
    writeComment(
  ...
  if (is.null(ctllist[["DoVar_adjust"]])) {
    ctllist[["Var_adjust"]] <- 0
  }
  ...
  wl(
    "N_lambdas",
    comment = "number of changes to make to default Lambdas (default value is 1.0)"
  )
  writeComment(
  ...
  if (ctllist[["more_stddev_reporting"]] != 0) {
    wl.vector(
      ...
      comment = "# selex type, len/age, year, N selex bins, Growth pattern, N growth ages, NatAge_area(-1 for all), NatAge_yr, N Natages"
    )
    ## Selex bin
    if (ctllist[["stddev_reporting_specs"]][4] > 0) {
      wl.vector(
        "stddev_reporting_selex",
        comment = "# selex bins to be reported (-1 in first bin to self-generate)"
      )
    }
    ## Growth bin
    if (ctllist[["stddev_reporting_specs"]][6] > 0) {
      wl.vector(
        ...
        comment = "# growth bins to be reported (-1 in first bin to self-generate)"
      )
    }
    ## N at age
