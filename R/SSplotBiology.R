...
SSplotBiology <-
  function(
    ...
    if (length(morphs) > nsexes) {
      cli::cli_warn(
        "!Error with morph indexing in SSplotBiology function. Code is not set up to handle multiple growth patterns or birth seasons."
      )
    }
    ...
    if (is.null(growthvaries)) {
      ...
      if (is.null(growthseries)) {
        cli::cli_warn(
          "No time-varying growth info because starter file set to produce limited report detail."
        )
      } else {
