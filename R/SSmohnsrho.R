...
SSmohnsrho <- function(summaryoutput, endyrvec = NULL, startyr = NULL){
	...
	print ("The expected order of models in the summary output are the reference model followed by retro -1, retro -2, and so forth.")
	...
	for(i in 1:(N-1))
      ...
      HmohnSSB[i] = (summaryoutput$SpawnBio[Hind, i + 1] - summaryoutput$SpawnBio[Hind, 1]) / summaryoutput$SpawnBio[Hind, 1]
      
      Hind = which(summaryoutput$recruits$Yr == endyrvec[i+1])   
      HmohnRec[i] = (summaryoutput$recruits[Hind, i + 1] - summaryoutput$recruits[Hind, 1]) / summaryoutput$recruits[Hind, 1]
      
      Hind = which(summaryoutput$Fvalue$Yr == endyrvec[i+1])   
      HmohnF[i] = (summaryoutput$Fvalue[Hind, i + 1] - summaryoutput$Fvalue[Hind, 1]) / summaryoutput$Fvalue[Hind, 1]
      
      Hind = which(summaryoutput$Bratio$Yr == endyrvec[i+1])   
      HmohnBratio[i] = (summaryoutput$Bratio[Hind, i + 1] - summaryoutput$Bratio[Hind, 1]) / summaryoutput$Bratio[Hind, 1]
	}
