...
read.admbFit <- function(file) {
  ...
  sublin <- lapply(strsplit(lin[1:ret[["totPar"]] + 2], " "), function(x) {
    x[x != ""]
  })
  ...
  corvec <- unlist(sapply(seq_along(sublin), function(i) {
    sublin[[i]][5:(4 + i)]
  }))
  ret[["cor"]][upper.tri(ret[["cor"]], diag = TRUE)] <- as.numeric(corvec)
  ret[["cor"]][lower.tri(ret[["cor"]])] <- t(ret[["cor"]])[lower.tri(ret[[
    "cor"
  ]])]
  # covariance matrix
