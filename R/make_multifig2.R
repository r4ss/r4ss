...
doSD <- TRUE ## length(ptsSD)==length(ptsx) & max(ptsSD) > 0

## Build data frame containing the key data. This data frame is
## calculated ahead of time and then used for determining ranges and
...
for(yr.temp in yrvec){
    for(bin in unique(dbase$Bin)){
        ...
        if(nrow(female)==1 & nrow(male)==1){
            e <- female$Exp/male$Exp
            o <- female$Obs/male$Obs
            ## Use delta method for multinomial estimators to get
            ## approximate SE for the ratio of the two (assuming expcted
            ## value of estimator is the MLE). See Casella and Berger
            ## p245.
            effN <- female$effN
            N <- female$N
            ## SS divides the observations by n so skip that
            pf <- female$Obs; pm <- male$Obs
            se.ratio <-
                sqrt((pf/pm)^2*( (1-pf)/(pf*N) + (1-pm)/(pm*N) +2/N ))
        } else {
            ## No ratio for this bin/length combination
            N <- effN <- e <- o <- se.ratio <- NA
        }
...
yrange <- c(0, max(c(df$Exp, df$Obs, df$Obs+0*df$se.ratio), na.rm=TRUE))
## reduce range to <= yupper (no impact if yupper=NULL)
yrange <- c(0, min(yrange[2],yupper))
...
for(ipanel in panelrange){
    ...
    points(x=df2$Bin, y=df2$Obs ,ylim=yrange_big, xlim=xrange_big, pch=16, col=gray(.5))
    segments(x0=df2$Bin, y0=df2$Obs-1.96*df2$se.ratio,
             y1=df2$Obs+1.96*df2$se.ratio, col=gray(.5))
    lines(df2$Bin, y=df2$Exp, lwd=2)
    ## add legends
