...
rm_dollar_sign <- function(file,
  ...
  if (allow_recursive) {
    ...
    while(length(grep(pattern_no_backtick, x = mod_lines)) > 0 &
          ind <= max_loops) {
      ind <- ind + 1
      mod_lines <- gsub(pattern = pattern_no_backtick, 
                        replacement = replace_no_backtick, 
                        mod_lines)
    }
    if (length(grep(pattern_no_backtick, x = mod_lines)) > 0) {
      warning("max_loops was set too low to replace all instances of dollar ",
              "sign references.")
    }
  } else {
    if (length(grep(pattern_no_backtick, x = mod_lines)) > 0) {
      warning("There are lists in lists, but allow_recursive = FALE, so not all", 
