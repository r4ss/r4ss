  ...
  function(
    ...
    if (
      ...
      for (ifleet in intersect(fleets, unique(discard[["Fleet"]]))) {
        ...
        if (!is.null(discard_spec)) {
          ...
          if (!ifleet %in% discard_spec[["Fleet"]]) {
            stop(
              "Fleet ",
              ifleet,
              " not found in table of discard specifications."
            )
          }
          # get degrees of freedom
          DF_discard <- discard_spec[["errtype"]][
            discard_spec[["Fleet"]] == ifleet
          ]
        }
        ...
        if (DF_discard == -3) {
          # truncated normal thanks to Robbie Emmet
        ...
        discard_units <- discard_spec[["units"]][
          discard_spec[["Fleet"]] == ifleet
        ]
        if (discard_units == 1) {
        ...
        for (isubplot in subplots) {
          ...
          if (plot) {
            dfracfunc(
              addfit = addfit,
              log = ifelse(isubplot %in% 3:4, TRUE, FALSE)
            )
          }
          if (print) {
            plotinfo <- save_png(
