...
make_multifig <-
  function(
    ...
    if (scalebins) {
      # if bin scaling turned on
      if (
        diff(range(allbins.obs)) > 0 && # & if the range of bins is non-zero
          length(allbins.obs) > 2 && # & if there are more than 2 bins
          length(unique(diff(allbins.obs)))
      ) {
        # & if there are different bin widths
    ...
    if (npages > 1 & ipage != 0) {
      panelrange <- intersect(
        panelrange,
        1:(nrows * ncols) + nrows * ncols * (ipage - 1)
      )
    }
    for (ipanel in panelrange) {
        ...
        getwidths <- function(ptsx) {
          if (length(ptsx) > 0) {
            ...
            for (ibin in seq_along(ptsx)) {
              widths[ibin] <- bin.width.table[["width"]][
                bin.width.table[["bin"]] == ptsx[ibin]
              ]
            }
          } else {
            widths <- NULL
      ...
      plot(
        ...
        ylim = yrange_big,
        xaxs = "i",
        yaxs = ifelse(bars, "i", "r"),
        ...
      )
      abline(h = 0, col = "grey") # grey line at 0
      if (linepos == 2) {
        # add lines behind points
      ...
      if (bub) {
        # if size input is provided then use bubble function
        ...
        if (sampsizeline > 0 && length(sampsize) > 0) {
          ...
          if (!is.null(sampsizemean)) {
            lines(
              rep(sampsizeline * sampsizemean, length(ptsy_i1_vec)),
              ptsy_i1_vec,
              col = 2,
              lty = 3
            )
        ...
        if (length(ptsx_i0) > 0) {
          ...
          if (polygons) {
            polygon(
              c(ptsx_i0[1], ptsx_i0, tail(ptsx_i0, 1)),
              c(0, ptsy_i0, 0),
        ...
        if (length(ptsx_i1) > 0) {
          ...
          if (polygons) {
            polygon(
              c(ptsx_i1[1], ptsx_i1, tail(ptsx_i1, 1)),
              c(0, ptsy_i1, 0),
        ...
        if (length(ptsx_i2) > 0) {
          ...
          if (polygons) {
            polygon(
              c(ptsx_i2[1], ptsx_i2, tail(ptsx_i2, 1)),
              c(0, ptsy_i2, 0),
        ...
        if (doSD) {
          ...
          if (length(ptsx_i0) > 0) {
            arrows(
              x0 = ptsx_i0,
              y0 = qnorm(p = 0.05, mean = ptsy_i0, sd = ptsSD_i0),
              x1 = ptsx_i0,
              y1 = qnorm(p = 0.95, mean = ptsy_i0, sd = ptsSD_i0),
              length = 0.01,
              angle = 90,
              code = 3,
              col = ptscol
          ...
          if (length(ptsx_i1) > 0) {
            arrows(
              x0 = ptsx_i1,
              y0 = qnorm(p = 0.05, mean = ptsy_i1, sd = ptsSD_i1),
              x1 = ptsx_i1,
              y1 = qnorm(p = 0.95, mean = ptsy_i1, sd = ptsSD_i1),
              length = 0.01,
              angle = 90,
              code = 3,
              col = ptscol
          ...
          if (length(ptsx_i2) > 0) {
            arrows(
              x0 = ptsx_i2,
              y0 = qnorm(p = 0.05, mean = ptsy_i2, sd = ptsSD_i2),
              x1 = ptsx_i2,
              y1 = qnorm(p = 0.95, mean = ptsy_i2, sd = ptsSD_i2),
              length = 0.01,
              angle = 90,
              code = 3,
              col = ptscol
      ...
      if (npanels == 1 | ipanel %% (nrows * ncols) == 1) {
        ...
        if (npanels > 1) {
          title(
            ...
            line = c(2, 0, 3, 3),
            outer = TRUE,
            cex.main = cex.main * fixcex
          )
          title(xlab = xlab, outer = TRUE, cex.lab = fixcex)
          title(
            ylab = ylab,
            line = ifelse(ylas %in% 1:2, max(3, 2 + .4 * maxchar_yaxs), 3.5),
            outer = TRUE,
            cex.lab = fixcex
          )
        } else {
