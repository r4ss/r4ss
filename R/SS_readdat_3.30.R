...
SS_readdat_3.30 <-
  ...
  datlist$Nareas <- get.val(dat, ind)
    ...
    if(!is.null(datlist$lencomp)){
      colnames(datlist$lencomp) <-
        c("Yr", "Seas", "FltSvy", "Gender", "Part", "Nsamp",
          if(datlist$Nsexes == 1){paste0("l", datlist$lbin_vector)}else{NULL},
          if(datlist$Nsexes > 1){c(paste0("f", datlist$lbin_vector),
    ...
    if(!is.null(datlist$agecomp)){
      colnames(datlist$agecomp) <-
        c("Yr", "Seas", "FltSvy", "Gender",
          ...
          if(datlist$Nsexes == 1){paste0("a", datlist$agebin_vector)}else{NULL},
          if(datlist$Nsexes > 1){c(paste0("f", datlist$agebin_vector),
  ...
  if(datlist$use_MeanSize_at_Age_obs){
    ...
    colnames(datlist$MeanSize_at_Age_obs) <-
      c("Yr", "Seas", "FltSvy", "Gender", "Part", "AgeErr", "Ignore",
        ...
        if(datlist$Nsexes > 1){c(paste0("f", datlist$agebin_vector),
                             paste0("m", datlist$agebin_vector))}else{NULL},
        if(datlist$Nsexes == 1){paste0("N_a", datlist$agebin_vector)}else{NULL},
        if(datlist$Nsexes > 1){c(paste0("N_f", datlist$agebin_vector),
  ...
  if(datlist$N_sizefreq_methods){
    ...
    for(imethod in seq_len(datlist$N_sizefreq_methods)) {
      Ncols <- 7 + datlist$Nsexes * datlist$nbins_per_method[imethod]
