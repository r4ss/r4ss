...
SS_readctl_3.30 <- function(
  ...
  if (!use_datlist) {
    ...
    if (datlist[["lbin_method"]] == 1) {
      ...
      ctllist[["Npopbins"]] <- Npopbins <-
        (datlist[["maximum_size"]] - datlist[["minimum_size"]]) /
        (datlist[["binwidth"]]) +
        1
      ...
      cli::cli_abort(
        "datlist[['lbin_method']] is {datlist[['lbin_method']]}, but only can be 1, 2, or 3."
      )
    }
    ...
    if (
      !is.null(datlist[["N_ageerror_definitions"]]) &&
        datlist[["N_ageerror_definitions"]] > 0
    ) {
      ctllist[["Do_AgeKey"]] <- ifelse(
        any(
          datlist[["ageerror"]][1:(nrow(datlist[["ageerror"]]) / 2) * 2, 1] < 0
        ),
        1,
        0
      )
  ...
  if (
    ...
    cli::cli_abort(
      "recr_dist_method 4 should only be used when GPxSettlementxArea=1."
    )
  }
  ...
  if (ctllist[["natM_type"]] == 0) {
    ...
    ctllist <- add_df(
      ctllist,
      name = "natM",
      nrow = ctllist[["N_GP"]] * abs(ctllist[["Nsexes"]]),
      ncol = Nages + 1,
  ...
  if (ctllist[["GrowthModel"]] == 1) {
    # 1=vonBert with L1&L2
    N_growparms <- 5
  } else if (ctllist[["GrowthModel"]] %in% c(2, 8)) {
    # 2=Richards with L1&L2, growth cess.
    N_growparms <- 6
  } else if (ctllist[["GrowthModel"]] %in% c(3, 4, 5)) {
    # 3,4=age_specific_K
    ...
    cli::cli_abort(
      "Growth Model {ctllist[['GrowthModel']]} is not supported yet"
    )
  }
  ...
  if (ctllist[["maturity_option"]] %in% c(3, 4)) {
    ...
    ctllist <- add_df(
      ctllist,
      name = "Length_Maturity",
      nrow = ctllist[["N_GP"]],
      ncol = Npopbins,
      col.names = paste0("Len_", 1:Npopbins)
    )
  }
  ...
  for (i in seq_len(abs(ctllist[["Nsexes"]]))) {
    for (j in seq_len(ctllist[["N_GP"]])) {
      if (N_natMparms > 0) {
        MGparmLabel[1:N_natMparms + cnt - 1] <- paste0(
          "NatM_p_",
          1:N_natMparms,
          "_",
          SexLabel[i],
      ...
      if (ctllist[["GrowthModel"]] == 1) {
        ...
        tmp <- c(
          ...
          "L_at_Amax",
          "VonBert_K",
          paste0("Age_K_", ctllist[["Age_K_points"]]),
          "CV_young",
          "CV_old"
        )
        MGparmLabel[1:(5 + Age_K_count) + cnt - 1] <- paste0(
          tmp,
          "_",
          SexLabel[i],
          "_GP_",
      ...
      MGparmLabel[cnt] <- paste0("Wtlen_1_", SexLabel[i], "_GP_", j)
      cnt <- cnt + 1
      MGparmLabel[cnt] <- paste0("Wtlen_2_", SexLabel[i], "_GP_", j)
      cnt <- cnt + 1
      if (i == 1) {
        MGparmLabel[cnt] <- paste0("Mat50%_", SexLabel[1], "_GP_", j)
        cnt <- cnt + 1
        MGparmLabel[cnt] <- paste0("Mat_slope_", SexLabel[1], "_GP_", j)
        cnt <- cnt + 1
        # Egg parameters, there are always 2, but what they are depends on the fecundity option
        MGparmLabel[cnt] <- paste0("Eggs_alpha_", SexLabel[1], "_GP_", j)
        cnt <- cnt + 1
        MGparmLabel[cnt] <- paste0("Eggs_beta_", SexLabel[1], "_GP_", j)
        cnt <- cnt + 1
      }
    }
  }
      ...
      if (ctllist[["recr_dist_inx"]]) {
        ...
        for (i in seq_len(ctllist[["N_GP"]])) {
          for (j in seq_len(ctllist[["N_areas"]])) {
            for (k in seq_len(ctllist[["nseas"]])) {
              # goes with settle
              tmp_lab_inx <- paste0(
                "RecrDist_interaction_GP_",
                i,
                "_area_",
                j,
                "_settle_",
  ...
  MGparmLabel[cnt + 1:ctllist[["N_GP"]] - 1] <- paste0(
    "FracFemale_GP_",
    1:ctllist[["N_GP"]]
  )
  ...
  if (use_datlist == TRUE) {
    ...
    if (isTRUE(is.null(predM_fleets))) {
      ...
      if (length(pred_fleet_lines) > 0) {
        # get the fleets
        tmp_pred_flts <- strsplit(
          ctl_with_cmts[pred_fleet_lines],
          split = "PredM2_"
        )
  ...
  if (
    ...
    cli::cli_warn(
      "There are time varying MG parameters, and AUTOGEN for MG is 0, so not expecting any short parameter lines."
    )
  }
  if (
    any(ctllist[["MG_parms"]][, c("env_var&link", "dev_link", "Block")] != 0) &
  ...
  N_seas_effects <- length(ctllist[["MGparm_seas_effects"]][
    ctllist[["MGparm_seas_effects"]] > 0
  ])
  if (N_seas_effects > 0) {
    ctllist <- add_df(
      ctllist,
      "MG_parms_seas",
      nrow = N_seas_effects * ctllist[["nseas"]],
      ncol = 7,
      col.names = srt_par_colnames
    )
  }
  ...
  if (
    ...
    cli::cli_warn(
      "There are time varying SR parameters, and AUTOGEN for SR is 0, so not expecting any short parameter lines."
    )
  }
  if (
    any(ctllist[["SR_parms"]][, c("env_var&link", "dev_link", "Block")] != 0) &
  ...
  if (use_datlist == FALSE && is.null(N_rows_equil_catch)) {
    ...
    if (parm_error) {
      cli::cli_abort(
        "Could not determine the number of initial F params from control file comments. Please specify N_rows_equil_catch instead of leaving NULL or read from a data list object by setting use_datlist = TRUE and specifying the datlist"
      )
    }
  }
  # get the comments. This will determine how big the df is.
  comments_initF <- NULL
  if (use_datlist == FALSE && isTRUE(N_rows_equil_catch > 0)) {
    # use approach that doesn't need data file.
  ...
  if (use_datlist) {
    if (any(datlist[["catch"]][, "year"] == -999)) {
      ...
      if (any(tmp_equil[, "catch"] > 0)) {
        ...
        tmp_equil_sort <- tmp_equil[
          order(tmp_equil[["fleet"]], tmp_equil[["seas"]]),
        ]
        comments_initF <- paste0(
          "InitF_seas_",
          tmp_equil_sort[["seas"]],
          "_flt_",
          tmp_equil_sort[["fleet"]],
  ...
  if (!is.null(ctllist[["Q_options"]])) {
    if (all(ctllist[["Q_options"]][["fleet"]] %in% 1:Nfleets)) {
      rownames(ctllist[["Q_options"]]) <- ctllist[["fleetnames"]][ctllist[[
        "Q_options"
      ]][["fleet"]]]
    } else {
      cli::cli_abort(
        "There was a error reading the Q_options, possibly due to the presence of initial F params prior to that input. Check that the line 'initial_F_parms; count = ' is correct."
      )
    }
  }
  # q parlines ----
  ...
  if (!is.null(ctllist[["Q_options"]])) {
    ...
    for (j in seq_len(nrow(ctllist[["Q_options"]]))) {
      if (
        (ctllist[["Q_options"]][j, ][["float"]] == 0) ||
          (ctllist[["Q_options"]][j, ][["float"]] == 1)
      ) {
        # handle float 0 or 1 as 1 parm sel
        flname <- fleetnames[ctllist[["Q_options"]][j, ][["fleet"]]]
        if (ctllist[["Q_options"]][j, ][["link"]] == 5) {
          comments_Q_type[[i]] <- paste0(
            "Q_base_",
            flname,
            "(",
      ...
      if (ctllist[["Q_options"]][j, ][["link"]] == 4) {
        # mirrored with offset
        flname <- fleetnames[ctllist[["Q_options"]][j, ][["fleet"]]]
        comments_Q_type[[i]] <- paste0(
          "Q_Mirror_offset_",
          flname,
      ...
      if (ctllist[["Q_options"]][j, ][["extra_se"]] == 1) {
        # do extra se
        flname <- fleetnames[ctllist[["Q_options"]][j, ][["fleet"]]]
        comments_Q_type[[i]] <- paste0(
          "Q_extraSD_",
  ...
  if (N_Q_parms > 0) {
    ...
    if (
      ...
      cli::cli_warn(
        "There are time varying q parameters, and AUTOGEN for q is 0, so not expecting any short parameter lines."
      )
    }
    if (
      any(ctllist[["Q_parms"]][, c("env_var&link", "dev_link", "Block")] != 0) &
  ...
  make_sel_lab <- function(
    ...
    if (!sel_type %in% c("s", "a")) {
      cli::cli_abort(
        "sel_type cannot be {sel_type}. Please change sel_type to 's' for size or 'a' for age."
      )
    }
    if (sel_type == "s") {
      sel_name <- "SizeSel"
    ...
    if (!is.null(n_labs)) {
      if (length(labs) != n_labs) {
        ...
        cli::cli_warn(
          "the length of labs generated was not equal to the prespecifiedn_labs"
        )
      }
    }
    labs
  ...
  for (j in seq_len(Nfleets)) {
    ...
    if (is.na(tmp_size_selex_Nparms)) {
      cli::cli_abort(
        "Pattern {as.character(ctllist[['size_selex_types']][j, 'Pattern'])} was used for the size selectivity pattern fleet or survey, but it is not valid."
      )
    }
      ...
      size_selex_label[[j]] <-
        c(
          ...
          make_sel_lab(
            "s",
            "Spline_Knot",
            1:ctllist[["size_selex_types"]][j, "Special"],
            jn,
            j
          ),
          make_sel_lab(
    ...
    if (ctllist[["size_selex_types"]][j, "Discard"] == 4) {
      # add 7 dome shaped retention parameters
      size_selex_label[[j]] <- c(
        size_selex_label[[j]],
    ...
    if (ctllist[["size_selex_types"]][j, "Discard"] %in% c(2, 4)) {
      # add 4 discard mortality parameters
      size_selex_label[[j]] <- c(
        size_selex_label[[j]],
    ...
    if (ctllist[["size_selex_types"]][j, "Male"] %in% 3) {
      # has value 3 or 5 - differs by select pattern
    ...
    } else if (ctllist[["size_selex_types"]][j, "Male"] %in% 4) {
      # has value 3 or 5 - differs by select pattern
      if (ctllist[["size_selex_types"]][j, "Pattern"] == 1) {
        size_selex_label[[j]] <- c(
          size_selex_label[[j]],
  ...
  for (j in seq_len(Nfleets)) {
    ...
    # do extra retention parameters (4 extra parameters)
    if (ctllist[["age_selex_types"]][j, "Discard"] %in% 1:2) {
      # add 4 retention parameters
      age_selex_label[[j]] <- c(
        age_selex_label[[j]],
    ...
    # note that for Discard = 3, no extra parameters are needed.
    if (ctllist[["age_selex_types"]][j, "Discard"] == 4) {
      # add 7 dome shaped retention parameters
      age_selex_label[[j]] <- c(
        age_selex_label[[j]],
    ...
    if (ctllist[["age_selex_types"]][j, "Discard"] %in% c(2, 4)) {
      # add 4 discard mortality parameters
      age_selex_label[[j]] <- c(
        age_selex_label[[j]],
    ...
    if (ctllist[["age_selex_types"]][j, "Male"] %in% 3) {
      # has value 3 or 5 - differs by select pattern
    ...
    } else if (ctllist[["age_selex_types"]][j, "Male"] %in% 4) {
      # has value 3 or 5 - differs by select pattern
      if (age_patterns[j] == "20") {
        age_selex_label[[j]] <- c(
          age_selex_label[[j]],
  ...
  if (use_datlist == TRUE) {
    ...
    if (
      any(datlist[["len_info"]][["CompError"]] > 0) |
  ...
  if (
    ...
    cli::cli_warn(
      "There are time varying size selectivity  parameters, and AUTOGEN for selectivity is 0, so not expecting any short parameter lines."
    )
  }
  if (
    any(
  ...
  if (ctllist[["Use_2D_AR1_selectivity"]] == 1) {
    add_2dar <- function(x) {
      ...
      specs2D <- as.data.frame(
        matrix(x$.dat[x$.i:(x$.i + 10)], nrow = 1),
      ...
      # read parameter rows
      par2D <- as.data.frame(
        matrix(x$.dat[x$.i + 1:n_par_vals - 1], nrow = n_pars, byrow = TRUE),
        stringsAsFactors = FALSE
      )
      colnames(par2D) <- c(
        "LO",
  ...
  if (ctllist[["TG_custom"]]) {
    ...
    ctllist <- add_df(
      ...
      ncol = 14,
      col.names = lng_par_colnames,
      comments = paste0("_TG_Loss_init_", 1:ctllist[["N_tag_groups"]])
    )
    ctllist <- add_df(
      ctllist,
      ...
      ncol = 14,
      col.names = lng_par_colnames,
      comments = paste0("TG_Loss_chronic_", 1:ctllist[["N_tag_groups"]])
    )
    ctllist <- add_df(
      ctllist,
      ...
      ncol = 14,
      col.names = lng_par_colnames,
      comments = paste0("TG_overdispersion_", 1:ctllist[["N_tag_groups"]])
    )
    ctllist <- add_df(
      ctllist,
      ...
      ncol = 14,
      col.names = lng_par_colnames,
      comments = paste0("TG_report_fleet_par_", 1:Ntag_fleets)
    )
    ctllist <- add_df(
      ctllist,
      ...
      ncol = 14,
      col.names = lng_par_colnames,
      comments = paste0("TG_report_decay_par_", 1:Ntag_fleets)
    )
  }
  ...
  if (ctllist[["N_lambdas"]] > 0) {
    ...
    if (any(chk1)) {
      # there are duplicates; don't remove. SS will just use the last one.
      warn_comp <- ctllist[["lambdas"]][chk1, "like_comp"]
      warn_flt <- ctllist[["lambdas"]][chk1, "fleet"]
      warn_phz <- ctllist[["lambdas"]][chk1, "phase"]
      cli::cli_warn(
        "Duplicate lambda input for likelihood component(s): {paste(warn_comp, sep = '', collapse = ', ')}; fleet(s): {paste(warn_flt, sep = '', collapse = ', ')}; phase(s): {paste(warn_phz, sep = '', collapse = ', ')}"
      )
    }
  ...
  if (ctllist[["more_stddev_reporting"]] != 0) {
    if (ctllist[["more_stddev_reporting"]] == 1) {
    ...
    } else if (ctllist[["more_stddev_reporting"]] == 2) {
      # adds option for M
      ...
      cli::cli_abort(
        "more_stddev_reporting read as {ctllist[['more_stddev_reporting']]}, but this is either not a valid SS option or not yet implemented in the SS_readctl function."
      )
    }
    ## Selex bin
    if (
    ...
    if (
      ...
      cli::cli_warn(
        "Additional stddev reporting being used with a model using empirical weight at age. Note that even if number of growth ages > 0, SS will ignore these and not expect any input for the line stddev_reporting_growth. Changing ctllist[['stdev_reporting_specs']][6] to 0 to make this clear."
      )
      ctllist[["stddev_reporting_specs"]][6] <- 0
    }
    if (
...
get_tv_parlabs <- function(full_parms, block_design) {
  ...
  for (i in loop_pars) {
    ...
    if (i %in% par_num[["block"]]) {
      ...
      if (n_blk > 0) {
        ...
        if (is.null(tmp_blk_design)) {
          cli::cli_abort(
            "Time blocks used in parameter setup, but not defined in the topof the control file. Please define the time blocks."
          )
        }
    ...
    if (i %in% par_num[["env"]]) {
      ...
      if (isTRUE(tmp_pat > 400 & tmp_pat < 500)) {
        # pattern 4 needs 2 pars, all else 1.
        parlab <- c(
          parlab,
          paste0(
...
translate_3.30_to_3.24_Q_setup <- function(
  ...
  Q_setup <- data.frame(
    matrix(data = 0, nrow = (Nfleets), ncol = 4),
    row.names = comments_fl
  )
  ...
  if (!is.null(Q_options)) {
    for (j in seq_len(nrow(Q_options))) {
      if (
        (Q_options[j, ][["float"]] == 0) || (Q_options[j, ][["float"]] == 1)
      ) {
        # handle float 0 or 1 as 1 parm sel
      ...
      if (Q_options[j, ][["link"]] == 2) {
        # mirrored
        Q_setup[Q_options[j, ][["fleet"]], ][["Q_type"]] <- -abs(Q_options[
          j,
        ][["link_info"]])
      }
      if (Q_options[j, ][["link"]] == 3) {
        # do power
        Q_setup[Q_options[j, ][["fleet"]], ][["Den_dep"]] <- 1
        i <- i + 1
        N_Q_parms <- N_Q_parms + 1
      }
      if (Q_options[j, ][["link"]] == 4) {
        # mirrored with offset
        Q_setup[Q_options[j, ][["fleet"]], ][["Den_dep"]] <- -abs(Q_options[
          j,
        ][["link_info"]])
        i <- i + 1
        N_Q_parms <- N_Q_parms + 1
      }
      if (Q_options[j, ][["extra_se"]] == 1) {
        # do extra se
