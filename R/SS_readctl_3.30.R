...
SS_readctl_3.30 <- function(file, verbose = FALSE, echoall = lifecycle::deprecated(),
  ...
  if (ctllist[["natM_type"]] == 0) {
    ...
    N_natMparms <- 1
    ctllist <- add_elem(ctllist, name = "Lorenzen_minage") ## Minimum age for survivorship based Lorenzen M
    ctllist <- add_elem(ctllist, name = "Lorenzen_maxage") ## Maximum age for survivorship based Lorenzen M
  }else {
