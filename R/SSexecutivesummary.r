...
SSexecutivesummary <- function(replist,
  ...
  if ("b" %in% tables) {
    ...
    if (format) {
      ...
      colnames(es.b) <- c(
        "Years", sb.label, "Lower Interval", "Upper Interval",
  ...
  if ("c" %in% tables) {
    ...
    if (format) {
      ...
      colnames(es.c) <- c(
        "Years", "Recruitment", "Lower Interval", "Upper Interval",
  ...
  if ("d" %in% tables) {
    ...
    if (format) {
      ...
      colnames(es.d) <- c(
        "Years", paste0(spr_type, " (%)"), "Interval",
        f_type, "Interval"
      )
    } else {
      es.d <- data.frame(
      ...
      colnames(es.d) <- c(
        "Years", spr_type, "Lower Interval", "Upper Interval",
