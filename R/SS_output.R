...
SS_output <-
  function(
    ...
    if (verbose) {
      ...
      if ((maxnonblank + 1) < ncols) {
        cli::cli_inform(paste0("Got all columns. To speed code, use ncols = ", maxnonblank + 1, " in the future."))
      }
    ...
    if (covar) {
      ...
      if (any(is.na(stdtable[["std"]]))) {
        cli::cli_warn(paste0("NA value for parameter uncertainty found in ", sum(is.na(stdtable[["std"]])), " rows of covar.sso file. ", "First par with NA: ", stdtable[["name"]][is.na(stdtable[["std"]])]))
      }
