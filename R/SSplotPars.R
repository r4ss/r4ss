  ...
  function(
    ...
    if (!showdev) {
      ...
      if (length(devrows) > 0) {
        ...
        if (length(goodnames) == 0) {
          cli::cli_n("no parameters to plot")
          return()
        }
      }
    } else {
    ...
    if (is.null(strings) & verbose) {
      messagetext <- glue::glue(
        "Plotting distributions for {npars} estimated parameters."
      )
      if (!showdev) {
        messagetext <- gsub(
          pattern = ".",
    ...
    for (ipar in 1:npars) {
      ...
      if (print & (nrows * ncols == 1 | ipar %% (nrows * ncols) == 1)) {
        ...
        if (debug) {
          cli::cli_alert_info("saving file to {file.path(plotdir, file)}")
        }
