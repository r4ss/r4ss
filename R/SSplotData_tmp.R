...
##                 # new code 3/9/16 which should be more robust to repeat values
##                 # such as for mean length-at-age data
##                 yrs <- sort(unique(allyrs))
##                 size <- rep(NA, length(yrs))
##                 for(iyr in 1:length(yrs)){
##                   if(length(grep("dbase",typename))>0){
##                     size.yrdat$N[dat$Fleet==ifleet]
##                   }
##                   size <- 
##                 }
##                 typetable_newrows <- data.frame(yr=yrs, fleet=ifleet,
##                                                 ntype=ntypes, typename=typename,
##                                                 size=size.sorted,
##                                                 stringsAsFactors=FALSE)
##                 typetable <- rbind(typetable, typetable_newrows)
...
## print(data.frame(x,y,sqrt(size.cex)*maxsize,
##                  adjustcolor(fleetcol[fleets==ifleet], alpha.f=alphasize)))
## #print(typetable)              
