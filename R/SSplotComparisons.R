  ...
  function(
    ...
    for (iline in (1:nlines)[mcmcVec]) {
      ...
      if (length(tmp) > 0) {
        ...
        SpawnBioLower[, imodel] <- lower[match(
          SpawnBioLower[["Label"]],
          mcmclabs
        )]
        SpawnBioUpper[, imodel] <- upper[match(
          SpawnBioUpper[["Label"]],
          mcmclabs
        )]
      }
      ...
      if (length(tmp) > 0) {
        ...
        SmryBioLower[, imodel] <- lower[match(
          SmryBioLower[["Label"]],
          mcmclabs
        )]
        SmryBioUpper[, imodel] <- upper[match(
          SmryBioUpper[["Label"]],
          mcmclabs
        )]
      }
      ...
      if (length(tmp) > 0) {
        ...
        SPRratioLower[, imodel] <- lower[match(
          SPRratioLower[["Label"]],
          mcmclabs
        )]
        SPRratioUpper[, imodel] <- upper[match(
          SPRratioUpper[["Label"]],
          mcmclabs
        )]
      }
      ...
      if (length(tmp) > 0) {
        ...
        recruitsLower[, imodel] <- lower[match(
          recruitsLower[["Label"]],
          mcmclabs
        )]
        recruitsUpper[, imodel] <- upper[match(
          recruitsUpper[["Label"]],
          mcmclabs
        )]
      }
      ...
      if (length(tmp) > 0) {
        ...
        recdevsLower[, imodel] <- lower[match(
          recdevsLower[["Label"]],
          mcmclabs
        )]
        recdevsUpper[, imodel] <- upper[match(
          recdevsUpper[["Label"]],
          mcmclabs
        )]
      }
    ...
    plotBio <- function(option, show_uncertainty = TRUE) {
      ...
      if (!add) {
        abline(h = 0, col = "grey")
        if (tickEndYr) {
          # include ending year in axis labels
      ...
      if (spacepoints %in% c(0, 1, FALSE)) {
        ...
        if (type != "l") {
          ...
          for (iline in 1:nlines) {
            imodel <- models[iline]
            Bratio2[
              (Bratio2[["Yr"]] - initpoint) %% spacepoints !=
                (staggerpoints * iline) %% spacepoints,
              imodel
            ] <- NA
          }
      ...
      if (spacepoints %in% c(0, 1, FALSE)) {
        ...
        if (type != "l") {
          ...
          for (iline in 1:nlines) {
            imodel <- models[iline]
            SPRratio2[
              (SPRratio2[["Yr"]] - initpoint) %% spacepoints !=
                (staggerpoints * iline) %% spacepoints,
              imodel
            ] <- NA
          }
      ...
      if (sprtarg > 0) {
        if (isTRUE(SPRratioLabel == "1-SPR")) {
          ...
          if (
            ...
            mtext(
              side = 2,
              text = paste("(1-SPR)/(1-SPR_", 100 * sprtarg, "%)", sep = ""),
              line = par()[["mgp"]][1],
              col = par()[["col.lab"]],
              cex = par()[["cex.lab"]]
            )
      ...
      if (spacepoints %in% c(0, 1, FALSE)) {
        ...
        if (type != "l") {
          ...
          for (iline in 1:nlines) {
            imodel <- models[iline]
            recruits2[
              (recruits2[["Yr"]] %% spacepoints - initpoint) !=
                (staggerpoints * iline) %% spacepoints,
              imodel
            ] <- NA
          }
      ...
      if (show_uncertainty) {
        for (iline in 1:nlines) {
          ...
          if (uncertainty[imodel]) {
            ...
            arrows(
              x0 = xvec,
              y0 = as.numeric(recdevsLower[, imodel]),
              x1 = xvec,
              y1 = as.numeric(recdevsUpper[, imodel]),
              length = 0.01,
              angle = 90,
              code = 3,
              col = col[iline]
      ...
      for (iline in 1:nlines) {
        ...
        if (mcmcVec[iline]) {
          ...
          if (length(mcmcColumn) == 0) {
            message(
              "No columns selected from MCMC for '",
              parname,
              "' in model ",
              imodel
            )
          ...
          if (!is.na(parSD) && parSD > 0) {
            ...
            if (limit0) {
              xmin <- max(0, xmin)
            } # by default no plot can go below 0
            if (fix0 & !grepl("R0", parname)) {
              xmin <- 0
            } # include 0 if requested (except for log(R0) plots)
            ...
            # if no SD, at least make sure interval includes MLE estimate
      ...
      if (is.null(ymax)) {
        ...
        if (!add) {
          if (cumulative) {
            ...
            plot(
              ...
              xaxs = "i",
              yaxs = yaxs,
              ylim = c(0, 1.1 * ymax * densityscaley),
              xlab = xlab,
              ylab = ""
            )
          }
        ...
        for (iline in (1:nlines)[good]) {
          ...
          if (mcmcVec[iline]) {
            ...
            if (!cumulative) {
              ...
              y <- cumsum(mcmcDens[[iline]][["y"]]) /
                sum(mcmcDens[[iline]][["y"]])
            }
            y2 <- NULL
            for (ii in x2) {
          ...
          if (densitytails & doShade) {
            ...
            polygon(
              c(x.lower[1], x.lower, rev(x.lower)[1]),
              c(0, y.lower, 0),
              col = shadecol[iline],
              border = NA
            )
            polygon(
              c(x.upper[1], x.upper, rev(x.upper)[1]),
              c(0, y.upper, 0),
              col = shadecol[iline],
              border = NA
            )
          ...
          if (densitymiddle & doShade) {
            ...
            polygon(
              c(x.middle[1], x.middle, rev(x.middle)[1]),
              c(0, y.middle, 0),
              col = shadecol[iline],
              border = NA
            )
        ...
        if (xunits != 1) {
          message(
            "x-axis for ",
            parname,
            " in density plot has been divided by ",
            xunits,
            " (so may be in units of ",
            xlab2,
            ")"
          )
        }
        ...
        if (length(expandednames) == 0) {
          ...
          if (16 %in% subplots) {
            for (iplot in 1:ndensities) {
              ...
              if (print) {
                save_png_comparisons(paste(
                  "compare16_densities_",
                  name,
                  ".png",
                  sep = ""
                ))
