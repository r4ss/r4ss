...
#################################################################################
  ...
  plotCI <- function(x,z=x,y=NULL,uiw,liw=uiw,ylo=NULL,yhi=NULL,...,sfrac = 0.01,ymax=NULL)
  {
  ...
  if(1 %in% c(plot, print) | length(cohortlines)>0)
  {
	...
	legend(legendloc,bty="n", c("Females","Males"), lty=1, col = c("red","blue"))}
    }
...
if(nseasons == 1){ # temporarily disable multi-season plotting of time-varying growth
  ...
  if(length(intersect(c(15:21,24),c(plot,print)))>0){
    ...
    print(paste("  ",nrow(ladbase),"rows of mean length at age data"),quote=F)
    print(paste("  ",nrow(wadbase),"rows of mean weight at age data"),quote=F)
...
if(3==4){ # temporarily turning off plot
  ...
  if(21 %in% c(plot,print)){
    SSv3_plot_comps(datonly=F,kind="L@A",bub=T,verbose=verbose,fleets=fleets,
		    samplesizeplots=F,showsampsize=F,showeffN=F,
		    maxrows=maxrows,maxcols=maxcols,fixdims=fixdims,
		    png=(21%in%print),GUI=(21%in%plot),smooth=smooth,plotdir=plotdir,
		    maxneff=maxneff,cex.main=cex.main,...)
    SSv3_plot_comps(datonly=F,kind="W@A",bub=T,verbose=verbose,fleets=fleets,
		    samplesizeplots=F,showsampsize=F,showeffN=F,
		    maxrows=maxrows,maxcols=maxcols,fixdims=fixdims,
		    png=(21%in%print),GUI=(21%in%plot),smooth=smooth,plotdir=plotdir,
		    maxneff=maxneff,cex.main=cex.main,...)
    if(verbose) print("Finished plot 21: mean length at age and mean weight at age",quote=F)
    flush.console()
