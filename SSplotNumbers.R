    ...
    for(iarea in areas){
      for(m in 1:nsexes){
        for(iperiod in 1:length(period)){
          ...
          if(period[iperiod]=="B") periodtitle <- "Beginning" else
          if(period[iperiod]=="M") periodtitle <- "Middle" else
          stop("'period' input to SSplotNumbers should include only 'B' or 'M'")
          plottitle1 <- paste(periodtitle, " of year expected numbers at age",sextitle," in thousands (max=",max(resz),")",sep="")
